...
#function for outputting text onm the screen
...
def create_cushion(poly_dims):
    body = pymunk.Body(body_type= pymunk.Body.STATIC) #does not move
    body.position = ((0,0))
...
while run:
    ...
    #check if all the balls have stopped movinggg
    taking_shot = True
    for ball in balls:
    ...
    if taking_shot == True and game_running == True:
        ...
        y_dist = -(balls[-1].body.position[1] - mouse_pos[1]) #negative because pygame y coordinates increase down the screen
    ...
    if powering_up == True and game_running == True:
        ...
        #draw power bars
        for b in range(math.ceil(force / 2000)): #.ceil is used for rounding the value
